...
with tempfile.TemporaryDirectory(prefix='database-', dir=sys.argv[2]) as directory:
    ...
    settings = Settings(_env_file=None, environment='test', database_url=db_url,
                        content_export_dir=Path(directory), admin_origin='https://localhost:3446')
    ...
    with factory.begin() as db:
        ...
        db.add(AdminBusinessPermission(admin_id=1,can_grant_catalog_access=True,can_close_reconciliation=True,can_create_intervention=True))
    app = create_app(settings=settings, engine=engine, session_factory=factory)
    uvicorn.run(app, host='127.0.0.1', port=8126, access_log=False, log_level='warning')
    engine.dispose()
