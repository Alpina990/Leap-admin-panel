...
export default function Home({session}:{session:AdminSession}){
 ...
 return <main id="main-content" data-route={section} className={mobileDetail?'mobile-detail':''}>
  ...
  {mobileDetail&&<button className="mobile-back action" onClick={()=>{setMobileDetail(false);requestAnimationFrame(()=>document.querySelector(section==='Content'?'[data-pencil-name="Content Catalog Panel"]':section==='Learners'?'[data-pencil-name="Learner Directory List"]':'[data-pencil-name="Payments List Panel"]')?.scrollIntoView({block:'start'}));}}>Back to list</button>}
