...
const methods={login:'POST',session:'GET',logout:'POST',overview:'GET',learners:'GET',payments:'GET',payment:'GET',notes:'GET,POST',notifications:'GET',notification:'GET',export:'GET','content-catalog':'GET','content-lessons':'GET','content-lesson':'GET','content-write':'POST','content-import-review':'POST'};
...
export async function proxyAdmin(request,action,config,fetcher=fetch){
 ...
 if(request.method==='POST'){
  ...
  if(action==='login'||action==='notes'||action==='content-write'||action==='content-import-review'){
   if(!/^application\/json(?:\s*;.*)?$/i.test(request.headers.get('content-type')||''))return failure(422,'admin_validation','JSON required.');
   try{body=JSON.stringify((action==='content-import-review'?importInput:action==='content-write'?contentInput:action==='login'?loginSchema:noteInput).parse(JSON.parse(raw)));}catch{return failure(422,'admin_validation','Invalid login input.');}
 ...
 if(Object.hasOwn(errors,upstream.status)){
  ...
  if(upstream.status===403&&data?.error?.code==='admin_forbidden')return failure(403,'admin_forbidden',action==='content-write'?'Content writing permission required.':'Note writing permission required.');
 ...
 const data=(contentSchemas[action]??(action==='export'?report:action==='notifications'?notifications:action==='notification'?notification:action==='overview'?overview:action==='learners'?directory:action==='payments'?payments:action==='payment'?payment:action==='notes'?(request.method==='GET'?notes:note):session)).parse(JSON.parse(raw));
