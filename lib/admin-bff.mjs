import 'server-only';
import {analyticsSchemas,analyticsQueries,analyticsPath,analyticsIdentity} from './admin-analytics-contract.mjs';
...
export async function proxyAdmin(request,action,config,fetcher=fetch){
 ...
 for(const [key,value] of query){
  const allowed=analyticsQueries[action]??(action==='business-state'?['learnerId']:action==='business-cases'?['limit','offset']:action==='content-catalog'?['limit','offset']:action==='content-lessons'?['unitId','limit','offset']:action==='content-lesson'?['lessonId']:action==='export'?['resource','limit','offset','start','end','sort','direction',...(resource==='learners'?['username']:resource==='payments'?['status','method','learnerId','dateField','currency']:['kind','learnerId'])]:action==='notifications'?['limit','offset','learnerId','kind','start','end','sort','direction']:action==='notification'?['notificationId']:action==='learners'?['limit','offset','username','start','end','sort','direction']:action==='payments'?['limit','offset','status','method','learnerId','start','end','sort','direction','dateField','currency']:action==='payment'?['orderId']:action==='notes'&&request.method==='GET'?['learnerId','limit','offset']:[]);
  if(!allowed.includes(key)||query.getAll(key).length!==1)return failure(422,'admin_validation','Invalid query.');
  const valid=key==='activityStatus'?['completed','unfinished'].includes(value):key==='dateField'?['createdAt','paidAt'].includes(value):key==='currency'?value==='UZS':key==='cohort'?['all','new','returning'].includes(value):['unitId','lessonId'].includes(key)?value.length>=1&&value.length<=120:['start','end','day'].includes(key)?/^\d{4}-\d{2}-\d{2}$/.test(value)&&!isNaN(Date.parse(value))&&new Date(value).toISOString().slice(0,10)===value:key==='resource'?Object.hasOwn(sortKeys,value):key==='sort'?(sortKeys[resource]??[]).includes(value):key==='direction'?['asc','desc'].includes(value):key==='kind'?value.length>=1&&value.length<=40:key==='notificationId'?value.length>=1&&value.length<=36:key==='username'?value.length>=1&&value.length<=64:key==='orderId'?value.length>=1&&value.length<=36:key==='status'?['pending','paid','cancelled'].includes(value):key==='method'?['payme','click','uzum','paylov'].includes(value):key==='learnerId'?/^-?\d{1,19}$/.test(value)&&BigInt(value)>=-(2n**63n)&&BigInt(value)<2n**63n:/^\d{1,7}$/.test(value)&&Number(value)>=(key==='limit'?1:0)&&Number(value)<=(key==='limit'?100:1000000);
  if(!valid)return failure(422,'admin_validation','Invalid query.');
 }
 if(['business-state','analytics-learner'].includes(action)&&!query.has('learnerId'))return failure(422,'admin_validation','Learner identifier required.');
 let body;
 if(request.method==='POST'){
 ...
 try{
 const outgoingQuery=new URLSearchParams(query);const analyticsRoute=Object.hasOwn(analyticsSchemas,action)?analyticsPath(action,outgoingQuery):null;
 const upstream=await fetcher(config.api+'/api/v1/admin/'+(analyticsRoute??(Object.hasOwn(businessSchemas,action)?businessPath(action,body):action))+(outgoingQuery.size?'?'+outgoingQuery.toString():''),{method:request.method,headers:outgoing,body,redirect:'error',cache:'no-store',signal:controller.signal});
 ...
 if(Object.hasOwn(errors,upstream.status)){
  ...
  if(action==='business-revoke'&&upstream.status===401&&data?.error?.code==='admin_reauthentication_failed')return failure(401,'admin_reauthentication_failed','Current administrator password is incorrect. Enter it again.');
 ...
 if(Object.hasOwn(analyticsSchemas,action)&&!analyticsIdentity(action,data,query))throw new Error('upstream analytics identity');
