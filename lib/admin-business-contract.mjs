...
export const businessSchemas={
 ...
 'business-state':z.object({learnerId,catalogGrant:catalog.nullable(),followUpVersion:count,permissions,followUpTasks:z.array(z.object({id:z.string().uuid(),learnerId,title:z.string().max(180),status:z.enum(['open','closed']),reason:z.string().max(1000),dueAt:timestamp.nullable(),version:count,createdAt:timestamp}).strict()).max(100)}).strict(),
 ...
 'business-grant':catalog.extend({preservedSectionEntitlements:z.array(z.string().max(120)).max(10000)}),
...
export function businessPath(action,body){return action==='business-permissions'?'business/permissions':action==='business-state'?'business/learner-state':action==='business-cases'?'business/reconciliation-cases':action==='business-grant'?'business/catalog-grants':action==='business-intervention'?'business/interventions':`business/reconciliation-cases/${encodeURIComponent(JSON.parse(body).caseId)}/close`;}
